...
def name_label(path):
    """
    go throught all json-stat files in a folder and 
    open it save the dataset.label and the file name to a file
...
# create the file "datasets" with all of the names of all the features 
#name_label("/home/dave/cso_meta/cso_json")
#write_json(names, "all_names")

# read all of the data sets and create a json with 
# Statistic.unit.statistic.Base key
# Staistic as value
#{
#   "000 Head" : ["AEA02C01", "AEA02C02",...],
#   "Million Litres": ["AEA02c04", ...],
#   "000 Tonnes" : ["AEA02C07"...],
# also
# dataset key
# statistic value
...
def unit_by_statistic(path):

    os.chdir(path)
    file_dict ={}
    for files in glob.glob("*"):
        if files == "parse_meta.py" or files == "stats.json":
            # do not read this source file or the file we are writing
            continue
        # load a dataset
        json_obj = read_file(files)
        if json_obj: # if it is availble
           try:
               ...
               print "error in ", files
               continue
    write_json(file_dict, "stats.json")
